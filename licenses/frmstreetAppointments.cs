...
{
    public partial class frmstreetAppointments : Form
    {
        private int _LDLAi;
        private int _PassedTests;
        private int _UserID;
        DataTable dt = new DataTable();
        public frmstreetAppointments(int lDLAi, int passedTests, int UserID)
        {
            InitializeComponent();
            _LDLAi = lDLAi;
        ...
        private void _Load()
        {
            ...
            foreach (DataRow AppointMentsRecordRow in AppointMentsTable.Rows)
            {
                if ((int)AppointMentsRecordRow["LocalDrivingLicenseApplicationID"] == _LDLAi &&
                        (int)AppointMentsRecordRow["TestTypeID"] == 3)
                {
        ...
        private void frmstreetAppointments_Load(object sender, EventArgs e)
        {
            _Load();
        }
